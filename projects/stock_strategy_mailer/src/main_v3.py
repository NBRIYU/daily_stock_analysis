...
from .screener_v3 import StockScreenerV3
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="A股五日线回踩策略邮件推送（排除当日涨停版）")
...
def main() -> None:
    ...
    screener = StockScreenerV3(min_score=min_score, max_candidates=max_candidates)
    ...
    subject = f"【A股策略选股-排除当日涨停】{run_date[:10]} 五日线回踩候选结果"
    ...
    print(
        ...
        f"最近涨停池截止日: {detailed['stats'].get('analysis_end_date', '')}"
    )
